...
$sth = $dbh->prepare( qq(SELECT COUNT(*) FROM news WHERE age='o') ) or die $dbh->errstr;
$sth->execute();
...
$sth = $dbh->prepare( qq(SELECT * FROM news WHERE age='o' ORDER BY num DESC) ) or die $dbh->errstr;
$sth->execute();
$i=0;
...
sub get_buts {
        ...
        if($ARGV[1]>1) {
        $j=$ARGV[1]-$buts2show;
        $buts.=qq(<A href="/cgi-bin/show_ar.pl?$ARGV[0]+$j"><< </A>);
        }
        else {
        $buts.='';
        }
        for ($i=$ARGV[1]; ($i<$ARGV[1]+$buts2show) && ($i<$total); $i++) {
                ...
                if($i==$ARGV[0]) {
                        ...
                        $buts.=qq(<A href="/cgi-bin/show_ar.pl?$i+$ARGV[1]">$i </A>$slash \n);
                }
        }
...
<tr><td align=center ID=heading>Архив <Font face="Wingdings">,</Font>
</td></tr>
</table>
<hr width=80%>
